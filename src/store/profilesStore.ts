...
interface ProfilesStore {
  ...
  searchProfiles: (
    query: string,
    filters?: { type?: "person" | "company"; companyType?: string }
  ) => Profile[];
  ...
  persist(
    (set, get) => ({
      ...
      fetchProfileConnections: async (profileId) => {
        ...
        try {
          const { data, error } = await supabase
            ...
            .select("*")
            .or(
              `source_profile_id.eq.${profileId},destination_profile_id.eq.${profileId}`
            )
            .eq("is_public", true)
            .eq("status", "approved");
          ...
          set((state) => ({
            connections: [
              ...state.connections.filter(
                (c) =>
                  c.sourceProfileId !== profileId &&
                  c.destinationProfileId !== profileId
              ),
              ...connections,
            ],
            isLoading: false,
      ...
      getSourceConnections: (profileId) => {
        return get().connections.filter(
          (conn) => conn.destinationProfileId === profileId
        );
      },

      getDestinationConnections: (profileId) => {
        return get().connections.filter(
          (conn) => conn.sourceProfileId === profileId
        );
      },
      ...
      getFeaturedProfiles: () => {
        ...
        return (
          get()
            .profiles// first sort by sponsor, then by MRR
            .sort((a, b) => {
              if (a.sponsor && !b.sponsor) return -1;
              if (!a.sponsor && b.sponsor) return 1;
              return (b.targetMrr || 0) - (a.targetMrr || 0);
            })
            .slice(0, 6)
        );
      },
      ...
      isProfileCached: (profileId) => {
        ...
        return (
          profiles.some((p) => p.id === profileId) && !!lastFetched[profileId]
        );
      },
